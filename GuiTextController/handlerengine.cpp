...
bool HandlerEngine::editRuleById(const std::string &id, const std::string &first_face, const std::string &third_face, Sex sex, Counting counting)
{
    this->controller->setCurrentTableName(known_hosts);
    ...
    auto search = nlohmann::json::object();
    search["_id"]=id;
...
bool HandlerEngine::deleteRule(const std::string &id)
{
    this->controller->setCurrentTableName(known_hosts);
    return this->controller->deleteDocument(id);
}
...
std::list<std::string> HandlerEngine::getAllRules(Sex sex, Counting counting) //Получить все правила по полу и числу
{
    this->controller->setCurrentTableName(known_hosts);
    ...
    case Sex::Male:
        {
            ...
            {
            case Counting::Single:
                search["first_face.male.single"]["$exists"]=true;
                break;
            case Counting::Plural:
                search["first_face.male.plural"]["$exists"]=true;
                break;
            }
        }
        break;
    case Sex::Female:
        {
            ...
            {
            case Counting::Single:
                search["first_face.female.single"]["$exists"]=true;
                break;
            case Counting::Plural:
                search["first_face.female.plural"]["$exists"]=true;
                break;
            }
        }
        break;
    ...
    //Возвращение результата из базы данных
    return controller->loadDocument(search.dump());
}
...
std::list<std::string> HandlerEngine::getAllSpecificRules(Sex sex, Counting counting)
{
    this->controller->setCurrentTableName(correction_hosts);
    std::string m_sex;
    std::string m_counting;
...
bool HandlerEngine::deleteSpecificRule(const std::string &id)
{
    this->controller->setCurrentTableName(correction_hosts);
    return controller->deleteDocument(id);
}
